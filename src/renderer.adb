...
package body renderer is
    ...
    function Generate_Polygon_Vertices(Sides : Positive; Radius : Natural; Center_X, Center_Y : Float) return Polygon is
    ...
    begin
        ...
        begin
            ...
            Current_Angle := Natural(Float(Center_X) + Float(Radius) * Ada.Numerics.Elementary_Functions.Cos(Angle));

            Vertices(I).X := Natural(Float(Center_X) + Float(Radius) * Ada.Numerics.Elementary_Functions.Cos(Angle));
            Vertices(I).Y := Natural(Float(Center_Y) + Float(Radius) * Ada.Numerics.Elementary_Functions.Sin(Angle));
        end;
        end loop;
        return Vertices;
    ...
    procedure Draw_Regular_Polygon (img : in out Image;
        ...
        Center_Y : float;
        c : Color) is
    Vertices : Polygon := Generate_Polygon_Vertices(Sides, Radius, Center_X, Center_Y);
    begin
        for I in Vertices'Range loop
            declare
